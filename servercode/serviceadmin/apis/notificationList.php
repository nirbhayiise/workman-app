...
$sql="select * from notification where n_status='1' ORDER BY n_id DESC";
		...
		else
		{
            ...
            $arr['response']['data'][$i]['n_id'] = $rowInfo['n_id'];
               $arr['response']['data'][$i]['n_title'] = $rowInfo['n_title'];
              $arr['response']['data'][$i]['n_message'] =$rowInfo['n_message'];
                     $arr['response']['data'][$i]['n_created'] =$rowInfo['n_created'];
